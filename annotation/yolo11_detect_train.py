        ...
        for data_dir in self.data_dirs:
            ...
            for view in self.views:
                ...
                assert img_dir.exists() and label_dir.exists(), (
                    f"Missing directory: {img_dir if not img_dir.exists() else label_dir}"
                )
...
class YoloDetectionTrainer:
    def __init__(
        self, data_yaml: str, model: str = "yolov8n.pt", epochs: int = 100, imgsz: int = 640, batch: int = 256
    ):
    ...
    def train(self):
        ...
        yolo.train(data=self.data_yaml, epochs=self.epochs, imgsz=self.imgsz, batch=self.batch)
